 ...
 * @author eliuzhe
 */
public class DeleteByNumberCommand extends AbstractCommand {

    /**
     * @param addressBook
     */
    public DeleteByNumberCommand(IAddressBook<AddressItem> addressBook) {
        super(addressBook);
    }
    ...
    public String execute(String cmdStr) {
        String ret = null;
        AddressItem item = addressBook.deleteItemByNumber(cmdStr);
        if (null != item) {
            ret = "One item " + item + "is deleted successfully";
        } else {
            ret = "Cannot find an item with phone number " + cmdStr;
        }
        return ret;
    }
