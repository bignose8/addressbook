 ...
 * @author eliuzhe
 */
public class LookupByNameCommand extends AbstractCommand {

    /**
     * @param addressBook
     */
    public LookupByNameCommand(IAddressBook<AddressItem> addressBook) {
        super(addressBook);
    }
    ...
    public String execute(String cmdStr) {
        String ret = null;
        AddressItem result = addressBook.lookupByName(cmdStr);
        if (null != result) {
            ret = "The look up result is " + result;
        } else {
            ret = "Cannot look up an item with name " + cmdStr;
        }
        return ret;
    }
